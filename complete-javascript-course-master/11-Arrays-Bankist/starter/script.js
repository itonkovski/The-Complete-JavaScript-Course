...
const calcAverageHumanAge = function (ages) {
  const calcDogAge = ages.map(age => (age <= 2 ? 2 * age : 16 + age * 4));
  const adultDogs = calcDogAge.filter(age => age >= 18);
  console.log(calcDogAge);
  console.log(adultDogs);

  const calcAvg = adultDogs.reduce((acc, age) => acc + age) / adultDogs.length;
